...
class Piece:
  ...
  def makeMove(self,file,rank,type):  
    ...
    if verifyVal is True:
      if self.hasMoved == 0:
        self.hasMoved = 1
        if self.type is ord('P') and abs(self.rank-rank) == 2:
          self.hasMoved = 2
      ...
      if self.type == ord('P') and board[rank-1][file-1] is None and self.file is not file:
        #EnPassant Code  
        for piece in [i for i in self.game.objects.values() if isinstance(i,Piece)]:
          if piece.file == file and abs(piece.rank - rank) == 1:
            targetPiece = piece
  ...
  def verifyMove(self,file,rank,type):
    ...
    if file < 1 or file > 8 or rank < 1 or rank > 8:
      return ("White" if self.game.playerID == 0 else "Black") + " Attempted to make a move for which the Destination was off of the chess board!"
    if self.file < 1 or self.file > 8 or self.rank < 1 or self.rank > 8:
      ...
      return ("White" if self.game.playerID == 0 else "Black") + " Attempted to make a move which did not move any piece!"
    elif self.owner != self.game.playerID:
        return ("White" if self.game.playerID == 0 else "Black") + " Tried to move a piece that belonged to " + ("White" if self.game.playerID is 1 else "Black")
    #Checks the final position. If it is an enemy piece the move is valid. If it is our own then it is invalid
      ...
      return ("White" if self.game.playerID == 0 else "Black") + " Attempted to take its own Piece!"
    ...
    #Move logic for a pawn
      ...
      if board[rank-1][file-1] is None:
        ...
        if self.file is not file:
          ...
          if abs(self.file-file) != 1:
            return  ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a pawn more than one square horizontally"
          elif not (rank+2*(self.owner-1) < 8 and rank+2*(self.owner-1) >= 0 and file-1 < 8 and file-1 >= 0):
            return ("White" if self.game.playerID == 0 else "Black") + " Attempted to attack an empty square"
            
          elif not (board[rank+2*(self.owner-1)][file-1] is not None \
    ...
    #Move logic for a rook
      ...
      if file == self.file:
        ...
        return ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a Rook in something other than a straight line!" 


    elif self.type == ord('N'):
      if not ((abs(self.rank-rank) == 2 and abs(self.file-file) == 1) or (abs(self.rank-rank) == 1 and abs(self.file-file) == 2)):
        return ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a Knight In something other than an L shape"
      ...
      #logic for castling
      if (not (self.hasMoved == 1)) and (self.rank == rank) and (abs(self.file-file) == 2):
        if file > self.file:
          for i in range(self.file, file): #Note, this is supposed to be self.file and file, not self.file-1 and file-1, think of it as self.file-1+1 and file-1+1 if it helps
            if board[rank-1][i] is not None:
              return ("White" if self.game.playerID == 0 else "Black") + " Attempted to castle, but there were pieces in the way! (piece was :" + chr(board[rank-1][i].type) + ") at " + str(rank) + "," + str(i+1)
          if board[rank-1][7] is None:
            return ("White" if self.game.playerID == 0 else "Black") + " Attempted to castle, but the Rook in question was either missing"
          else:
            if board[rank-1][7].owner is not self.owner or board[rank-1][7].type is not ord('R') or board[rank-1][7].hasMoved != 0:
              return ("White" if self.game.playerID == 0 else "Black") + " Attempted to castle, but the Rook in question had already moved"
        else:
     ...
     #normal move logic
        ...
        return ("White" if self.game.playerID == 0 else "Black") + " Attempted to move their King farther than it could move"
    ...
    if rank > self.rank:
      for i in range(self.rank , rank-1):
        if board[i][self.file-1] is not None:
          canMove = ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a piece, but there were other pieces in the way"
  ...
  def checkDiagonal(self,rank,file):
    ...
    if abs(self.rank-rank) is not abs(self.file-file):
      canMove = ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a non Knight piece in a move which was neither straight nor diagonal"
    #If the piece is moving to the right
    elif file > self.file:
      for i in range(self.file, file-1):
        #Moving to Up-Right
        if rank > self.rank:
          if board[self.rank + i - self.file][i] is not None:
            canMove = ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a piece diagonally, but there were other pieces in the way"
      ...
      for i in range(file, self.file - 1):
        ...
        if rank > self.rank:
          if board[rank - i + file -2][i] is not None:
            canMove = ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a piece diagonally, but there were other pieces in the way at rank " + chr(rank - i + file - 1) + " file " + chr(i+1)
        #Moving to Down-Left
        elif rank < self.rank:
